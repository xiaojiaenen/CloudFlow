...
export class AdminService {
  ...
  async createUser(payload: CreateUserDto) {
    ...
    const nextPassword = payload.password?.trim() || this.generateTemporaryPassword();
    const passwordAutoGenerated = !payload.password?.trim();
    ...
    try {
      welcomeEmailSent = await this.notificationService.sendWelcomeEmail({
        email: user.email,
        name: user.name,
        role: user.role,
    ...
    return {
      ...user,
      isSuperAdmin: this.isSuperAdminEmail(user.email),
      temporaryPassword: passwordAutoGenerated ? nextPassword : undefined,
      welcomeEmailSent,
    };
  }
  ...
  async resetUserPassword(id: string, payload: ResetUserPasswordDto) {
    ...
    let emailSent = false;
    try {
      emailSent = await this.notificationService.sendPasswordResetEmail({
        email: user.email,
        name: user.name,
        password: nextPassword,
      });
