...
import {
  createRecorderBrowserPool,
  replayRecorderActionsOnPage,
} from './runtime/recorder';
import {
  createRecorderActionId,
  createRecorderPrecheckIssue,
  findNextRecorderActionStart,
  findPreviousRecorderActionStart,
  getRecorderActionGroupLength,
  invalidateRecorderAnalysis,
  isRecorderWaitForUrlAction,
  normalizeRecorderParameterKey,
  RecorderActionRecord,
  RecorderSession,
  replaceRecorderActions,
  toRecorderActionSummary,
  truncateRecorderPreview,
} from './runtime/recorder-model';
import {
  captureRecorderSnapshot as captureRecorderSnapshotWithRedis,
  getRecorderImageKey,
  getRecorderSnapshotKey,
  publishRecorderResponse as publishRecorderResponseWithRedis,
  startRecorderScreenshotLoop as startRecorderScreenshotLoopWithRedis,
} from './runtime/recorder-redis';
...
async function publishRecorderResponse(
  ...
  return publishRecorderResponseWithRedis(
    publisher,
    requestId,
    result,
...
async function captureRecorderSnapshot(
  session: RecorderSession,
  delta?: RecorderSessionDelta,
) {
  return captureRecorderSnapshotWithRedis(
    publisher,
    session,
    RECORDER_SNAPSHOT_TTL_MS,
    delta,
...
function startRecorderScreenshotLoop(session: RecorderSession) {
  return startRecorderScreenshotLoopWithRedis(
    publisher,
    session,
...
const recorderBrowserPool = createRecorderBrowserPool(getBrowser, {
  maxIdlePages: 2,
...
function buildRecordedWorkflowDefinition(
  ...
  const inputSchema: WorkflowInputField[] = [];
  ...
  for (let index = 0; index < actions.length; index += 1) {
    ...
    const waitAction: Extract<RecorderActionRecord, { type: 'wait_for_url' }> | null =
      action.type !== 'wait_for_url' &&
      nextAction?.type === 'wait_for_url' &&
      nextAction.linkedActionId === action.id
        ? nextAction
        : null;
    const blockBaseY = 60 + blockIndex * 180;
    const blockBaseX = 240;
    ...
    if (action.type === 'open_page') {
      ...
      const useRuntimeInput =
        action.useRuntimeInput && action.parameterKey?.trim();
      const parameterKey = normalizeRecorderParameterKey(
        action.parameterKey || action.label || 'input_value',
      );

      if (useRuntimeInput && !inputKeys.has(parameterKey)) {
        inputKeys.add(parameterKey);
        inputSchema.push({
          key: parameterKey,
          label: action.parameterLabel?.trim() || action.label || '录制输入',
          type: 'text',
          required: true,
          placeholder: action.value,
          defaultValue: action.value,
          description:
            action.parameterDescription?.trim() ||
            `录制步骤“${action.label}”运行时输入值`,
        });
      }
    ...
    if (previousNodeId) {
      canvasEdges.push({
        id: `e-${previousNodeId}-${clientNodeId}`,
        source: previousNodeId,
        target: clientNodeId,
      });
    }

    previousNodeId = clientNodeId;
    blockIndex += 1;

    if (waitAction) {
      const waitNodeId = `recorded-node-${index + 2}`;
      nodes.push({
        clientNodeId: waitNodeId,
        type: 'wait_for_url',
        urlIncludes: waitAction.url,
        waitUntil: 'load',
        timeout: 10000,
        label: waitAction.label,
      } as WorkflowNode);
      canvasNodes.push({
        id: waitNodeId,
        type: 'custom',
      ...
      canvasEdges.push({
        id: `e-${clientNodeId}-${waitNodeId}`,
        source: clientNodeId,
        target: waitNodeId,
      });
      previousNodeId = waitNodeId;
      index += 1;
      blockIndex += 0;
    }
  }
...
async function createRecorderSession(
  ...
  const { context, page } = await recorderBrowserPool.acquire();
  ...
  try {
    ...
    await recorderBrowserPool.release(context, page).catch(() => undefined);
    throw error;
  }
}
...
async function navigateRecorderSession(
  ...
  invalidateRecorderAnalysis(session);
...
async function clickRecorderSession(
  ...
  session.actions.push(action);
  const waitAction = maybeCreateWaitForUrlAction(
    previousUrl,
    session.page.url(),
    action.id,
  );
...
async function updateRecorderAction(
  session: RecorderSession,
  payload: RecorderCommandPayload,
): Promise<RecorderCommandResult> {
  invalidateRecorderAnalysis(session);
  ...
  const actionIndex = session.actions.findIndex((action) => action.id === actionId);
  ...
  if (action.type === 'open_page' && payload.url?.trim()) {
    action.url = payload.url.trim();
  }

  if (action.type === 'click' && payload.selector?.trim()) {
    action.selector = payload.selector.trim();
  }
  ...
  if (action.type === 'input') {
    ...
    if (payload.value !== undefined) {
      action.value = String(payload.value);
    }

    if (payload.useRuntimeInput !== undefined) {
      action.useRuntimeInput = Boolean(payload.useRuntimeInput);
    }

    if (payload.parameterKey !== undefined) {
      action.parameterKey = payload.parameterKey?.trim()
        ? normalizeRecorderParameterKey(payload.parameterKey)
        : undefined;
    }

    if (payload.parameterLabel !== undefined) {
      action.parameterLabel = payload.parameterLabel?.trim() || undefined;
    }

    if (payload.parameterDescription !== undefined) {
      action.parameterDescription =
        payload.parameterDescription?.trim() || undefined;
    }
  }

  if (action.type === 'press_key' && payload.key?.trim()) {
    action.key = payload.key.trim();
  }
  ...
  if (action.type === 'scroll') {
    ...
    if (payload.distance !== undefined) {
      action.distance = Math.max(1, Number(payload.distance));
    }
  }

  if (action.type === 'wait_for_url' && payload.url?.trim()) {
    action.url = payload.url.trim();
  }
...
async function analyzeRecorderSession(
  session: RecorderSession,
): Promise<RecorderCommandResult> {
  const suggestions = await session.page.evaluate(() => {
    ...
    const makeSelector = (element: Element) => {
      ...
      if (element.id?.trim()) {
        return `#${CSS.escape(element.id.trim())}`;
      }

      const dataTestId = element.getAttribute('data-testid');
      if (dataTestId?.trim()) {
        return `[data-testid="${dataTestId.trim().replace(/"/g, '\\"')}"]`;
      }
    ...
    const push = (suggestion: Suggestion) => {
      if (!suggestion.selector || results.some((item) => item.selector === suggestion.selector)) {
        return;
      }
    ...
    document.querySelectorAll('table').forEach((table, index) => {
      if (results.length >= 6 || !visible(table)) {
        return;
      }

      const selector = `${makeSelector(table)} tbody tr`;
      const preview = toPreview((table.textContent || '').slice(0, 160));
      push({
        label: `提取表格第 ${index + 1} 组数据`,
        selector,
        property: 'text',
        targetMode: 'all',
        preview,
        saveKey: `table_rows_${index + 1}`,
        collectionKey: `table_rows_${index + 1}`,
        collectionName: `表格 ${index + 1}`,
        recommendedRecordMode: 'array',
      });
    });

    document
      .querySelectorAll('[data-list], [role="list"], ul, ol, .list, .card-list, .table-body')
      .forEach((container, index) => {
        if (results.length >= 6 || !visible(container)) {
          return;
        }

        const selector = `${makeSelector(container)} > *`;
        const preview = toPreview((container.textContent || '').slice(0, 160));
        push({
          label: `提取列表第 ${index + 1} 组内容`,
          selector,
          property: 'text',
          targetMode: 'all',
          preview,
          saveKey: `list_items_${index + 1}`,
          collectionKey: `list_items_${index + 1}`,
          collectionName: `列表 ${index + 1}`,
          recommendedRecordMode: 'array',
        });
      });

    document
      .querySelectorAll('h1, h2, h3, .title, .summary, .detail, [data-field], [data-label]')
      .forEach((element, index) => {
        if (results.length >= 8 || !visible(element)) {
          return;
        }
        ...
        push({
          label: `提取关键信息 ${index + 1}`,
          selector: makeSelector(element),
          property: 'text',
          targetMode: 'first',
          preview: toPreview(text),
          saveKey: makeKey(text) || `detail_${index + 1}`,
          recommendedRecordMode: 'single',
        });
      });
  ...
  session.suggestions = suggestions.map((item, index) => ({
    id: `rec-suggestion-${index + 1}-${Math.random().toString(36).slice(2, 7)}`,
    ...item,
  }));
...
async function precheckRecorderSession(
  session: RecorderSession,
): Promise<RecorderCommandResult> {
  const issues: RecorderPrecheckIssue[] = [];

  if (session.actions.length === 0) {
    issues.push(
      createRecorderPrecheckIssue('error', '当前没有可生成的录制步骤。'),
    );
  }

  const firstAction = session.actions[0];
  if (firstAction && firstAction.type !== 'open_page') {
    issues.push(
      createRecorderPrecheckIssue(
        'error',
        '录制流程缺少起始打开页面步骤。',
  ...
  const selectorChecks = new Map<string, string>();

  for (const action of session.actions) {
    if (action.type === 'input') {
      if (action.useRuntimeInput && !action.parameterKey?.trim()) {
        issues.push(
          createRecorderPrecheckIssue(
            'error',
            '参数化输入缺少参数标识。',
      ...
      if (action.useRuntimeInput && action.parameterKey?.trim()) {
        const key = normalizeRecorderParameterKey(action.parameterKey);

        if (parameterKeySet.has(key)) {
          issues.push(
            createRecorderPrecheckIssue(
              'warning',
              `参数标识 ${key} 重复，多个录制步骤会共用同一运行参数。`,
              action.id,
            ),
          );
        }
    ...
    if (
      ...
      selectorChecks.set(action.id, action.selector.trim());
    }
  }

  if (selectorChecks.size > 0) {
    const selectorCheckResults = await session.page.evaluate((selectors) => {
      return selectors.map(({ actionId, selector }) => {
        try {
          return {
            actionId,
            selector,
    ...
    }, Array.from(selectorChecks.entries()).map(([actionId, selector]) => ({ actionId, selector })));

    for (const result of selectorCheckResults) {
      if (result.count === -1) {
        issues.push(
          createRecorderPrecheckIssue(
            'error',
            `选择器格式无效：${result.selector}`,
            result.actionId,
          ),
        );
        continue;
      ...
      if (result.count === 0) {
        issues.push(
          createRecorderPrecheckIssue(
            'warning',
            `当前页面未匹配到元素：${result.selector}。这不一定是错误，但建议复核。`,
            result.actionId,
          ),
        );
        ...
        issues.push(
          createRecorderPrecheckIssue(
            'warning',
            `当前页面匹配到 ${result.count} 个元素：${result.selector}，后续执行可能不够稳定。`,
            result.actionId,
          ),
        );
      }
  ...
  session.precheckIssues = issues;
  const snapshot = await captureRecorderSnapshot(session, {
    kind: 'precheck_updated',
    timestamp: new Date().toISOString(),
  });
  ...
  return {
    ...
    sessionId: session.id,
    pageUrl: session.page.url(),
    precheckIssues: issues,
    snapshot: snapshot ?? undefined,
  };
}

async function resumeRecorderFromAction(
  session: RecorderSession,
  payload: RecorderCommandPayload,
): Promise<RecorderCommandResult> {
  invalidateRecorderAnalysis(session);
  ...
  if (isRecorderWaitForUrlAction(action)) {
    return {
      ok: false,
      sessionId: session.id,
      error: '自动等待步骤会跟随上一动作恢复，无需单独断点重录。',
    };
  }

  const keepLength = getRecorderActionGroupLength(session.actions, actionIndex);
  const actionsToReplay = session.actions.slice(0, actionIndex + keepLength);
  const { context: nextContext, page: nextPage } = await recorderBrowserPool.acquire();

  try {
    await replayRecorderActionsOnPage(nextPage, actionsToReplay);

    session.stopScreenshotLoop();
    await recorderBrowserPool
      .release(session.context, session.page)
      .catch(() => undefined);

    session.context = nextContext;
    session.page = nextPage;
    replaceRecorderActions(session, actionsToReplay);
    session.stopScreenshotLoop = startRecorderScreenshotLoop(session);
    ...
    await recorderBrowserPool
      .release(nextContext, nextPage)
      .catch(() => undefined);
    throw error;
  }
}

async function moveRecorderAction(
  session: RecorderSession,
  payload: RecorderCommandPayload,
): Promise<RecorderCommandResult> {
  invalidateRecorderAnalysis(session);
  const actionId = payload.actionId?.trim();
  const moveDirection = payload.moveDirection;
  ...
  const targetAction = session.actions[actionIndex];

  if (isRecorderWaitForUrlAction(targetAction)) {
    return {
      ok: false,
      sessionId: session.id,
      error: '自动等待步骤会跟随上一动作一起移动，无需单独调整。',
    };
  }

  if (actionIndex === 0 && targetAction.type === 'open_page') {
    return {
      ok: false,
      sessionId: session.id,
      error: '第一步打开页面是录制流程的起点，不能下移。',
    };
  }

  const targetIndex =
    moveDirection === 'up'
      ? findPreviousRecorderActionStart(session.actions, actionIndex)
      : findNextRecorderActionStart(session.actions, actionIndex);
  ...
  const currentLength = getRecorderActionGroupLength(session.actions, actionIndex);
  const currentGroup = session.actions.slice(
    actionIndex,
    actionIndex + currentLength,
  );

  if (moveDirection === 'up') {
    const previousLength = getRecorderActionGroupLength(session.actions, targetIndex);
    const previousGroup = session.actions.slice(
      targetIndex,
      targetIndex + previousLength,
    );
    const nextActions = [
      ...session.actions.slice(0, targetIndex),
      ...currentGroup,
      ...previousGroup,
      ...session.actions.slice(actionIndex + currentLength),
    ];

    replaceRecorderActions(session, nextActions);
  } else {
    const nextLength = getRecorderActionGroupLength(session.actions, targetIndex);
    const nextGroup = session.actions.slice(targetIndex, targetIndex + nextLength);
    const nextActions = [
      ...session.actions.slice(0, actionIndex),
      ...nextGroup,
      ...currentGroup,
      ...session.actions.slice(targetIndex + nextLength),
    ];
...
async function deleteRecorderAction(
  session: RecorderSession,
  payload: RecorderCommandPayload,
): Promise<RecorderCommandResult> {
  invalidateRecorderAnalysis(session);
  ...
  const action = session.actions[actionIndex];
  const deleteLength = getRecorderActionGroupLength(session.actions, actionIndex);

  if (
    actionIndex === 0 &&
    action.type === 'open_page' &&
    session.actions.length > deleteLength
  ) {
    return {
      ok: false,
      sessionId: session.id,
      error: '第一步打开页面不能删除，否则后续录制步骤将无法执行。',
    };
  }
  ...
  if (isRecorderWaitForUrlAction(action)) {
    ...
    session.actions.splice(actionIndex, deleteLength);
  }

  const snapshot = await captureRecorderSnapshot(session, {
    kind: 'actions_cleared',
    timestamp: new Date().toISOString(),
  });
...
async function clearRecorderActions(
  session: RecorderSession,
): Promise<RecorderCommandResult> {
  invalidateRecorderAnalysis(session);
...
async function finishRecorderSession(
  ...
  const mode: RecorderWorkflowMode = payload.mode ?? 'workflow';
  const recommendedName =
    payload.name?.trim() ||
    (mode === 'template'
      ? `${session.name || '录制工作流'}模板`
      : session.name || '录制工作流');
...
async function closeRecorderSession(
  ...
  await recorderBrowserPool
    ...
    .catch(() => undefined);
  await publisher
    .del(getRecorderSnapshotKey(session.id), getRecorderImageKey(session.id))
    .catch(() => undefined);
  ...
  try {
    ...
    switch (payload.type) {
      ...
      case 'scroll':
        ...
        break;
      case 'update_action':
        result = await updateRecorderAction(session, payload);
        break;
      case 'move_action':
        result = await moveRecorderAction(session, payload);
        break;
      case 'delete_action':
        result = await deleteRecorderAction(session, payload);
        break;
      case 'clear_actions':
        result = await clearRecorderActions(session);
        break;
      case 'resume_from_action':
        result = await resumeRecorderFromAction(session, payload);
        break;
      case 'analyze':
        result = await analyzeRecorderSession(session);
        break;
      case 'precheck':
        result = await precheckRecorderSession(session);
        break;
      case 'snapshot':
        result = {
          ok: true,
          ...
          pageUrl: session.page.url(),
          snapshot:
            (await captureRecorderSnapshot(session, {
              kind: 'snapshot',
              timestamp: new Date().toISOString(),
            })) ?? undefined,
        };
...
async function shutdown(code = 0) {
  ...
  await recorderBrowserPool.drain().catch(() => undefined);
